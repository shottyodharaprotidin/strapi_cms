  ...
  return {
    async findByArticle(ctx) {
      ...
      try {
        ...
        if (localeParam) whereClause.locale = localeParam;
        const rows = await strapi.db.query('plugin::comments.comment').findMany({
          where: whereClause,
          select: ['id','documentId','content','authorId','authorName','authorEmail','authorAvatar','createdAt','updatedAt','approvalStatus','locale'],
        ...
        const nodes = (rows || []).map((item: any) => ({
          ...
          author: { id: item.authorId, name: item.authorName || 'Anonymous', email: item.authorEmail, avatar: item.authorAvatar },
    ...
    async listForAdmin(ctx) {
      ...
      try {
        ...
        const docIds = [...new Set((rows || []).map((r: any) => {
          const match = (r.related || '').match(/api::article\.article:(.+)/);
          return match ? match[1] : null;
        }).filter(Boolean))] as string[];
        const articles = docIds.length
          ? await strapi.db.query('api::article.article').findMany({ where: { documentId: { $in: docIds } }, select: ['documentId','slug'] })
          : [];
        ...
        const data = (rows || []).map((r: any) => {
          ...
          return { id: r.id, authorName: r.authorName, authorEmail: r.authorEmail || '-', content: r.content, threadOf: r.threadOf?.id ?? null, postUrl, status: r.approvalStatus, locale: r.locale, createdAt: r.createdAt };
        });
    ...
    async createForArticle(ctx) {
      ...
      const body = (ctx.request.body || {}) as {
        ...
        content?: string; threadOf?: number | string | null; locale?: string;
      };
      if (!documentId) return ctx.badRequest('documentId is required');
      if (!body?.content || typeof body.content !== 'string' || !body.content.trim()) return ctx.badRequest('content is required');
      if (!body?.author?.name || !body?.author?.email) return ctx.badRequest('author name and email are required');
      ...
      try {
        ...
        if (body.threadOf !== null && body.threadOf !== undefined && body.threadOf !== '') {
          ...
          if (Number.isFinite(parsed) && parsed > 0) parentCommentId = parsed;
        }
        const created = await strapi.db.query('plugin::comments.comment').create({
          data: {
            ...
            blocked: false, blockedThread: false, removed: false,
            approvalStatus: 'PENDING',
            threadOf: parentCommentId,
          },
        });
        return {
          id: created.id, documentId: created.documentId, content: created.content,
          author: { id: created.authorId, name: created.authorName, email: created.authorEmail, avatar: created.authorAvatar },
